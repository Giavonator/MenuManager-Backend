---
timestamp: 'Fri Oct 24 2025 07:58:46 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251024_075846.547ab4e0.md]]'
content_id: 153464b23a44d3fdc1b0971387e7c55cacfaa43c0a95296d29a15dca85e68bf3
---
...
type ChangeRecipeScalingInput = { menu: ID; recipe: Recipe; newScalingFactor: number };
  ...
  async createMenu(
    ...
    try {
      ...
      if (existingMenu) {
        return {
          error:
            `A menu already exists for user ${actingUser} on ${toDateOnlyString(date)}.`,
        };
      }
    ...
    try {
      ...
      if (date !== undefined) {
        ...
        if (conflictMenu) {
          return {
            error:
              `Another menu already exists for user ${existingMenu.owner} on ${toDateOnlyString(date)}.`,
          };
        }
  ...
  async _getMenuByDate(
    ...
    try {
      ...
      if (!menuDoc) {
        return {
          error:
            `No menu found for user ${owner} on date ${toDateOnlyString(date)}.`,
        };
      }
      ...
      const errorMessage = e instanceof Error ? e.message : String(e);
      console.error(
        `Error getting menu by date ${toDateOnlyString(date)} for owner ${owner}: ${errorMessage}`,
      );
