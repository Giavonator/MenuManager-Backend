---
timestamp: 'Fri Oct 24 2025 08:07:10 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251024_080710.372a47f6.md]]'
content_id: 1c8e5084886d2c4633d53e8ffef8c90cbb66152693a5c9883fed3939beed174d
---
...
import { testDb, freshID, ID } from "@utils/database.ts";
import {
  assertAndLog,
  assertExistsAndLog,
  printStepHeader,
...
Deno.test("MenuCollectionConcept - Principle Verification", async (t) => {
  ...
  try {
    const christmas2024 = getNormalizedDate(2024, 12, 25);
    let menuId: ID;

    await t.step(
      "1. User Alice creates 'Christmas Dinner' menu for Dec 25, 2024",
      async () => {
        ...
        assertAndLog(
          ...
          christmas2024.toISOString().split("T")[0],
    ...
    await t.step("2. Add 'Turkey' and 'Gravy' recipes with scaling factors", async () => {
      const currentStepMsg =
      ...
      const menuRecipes = (recipesInMenu as { menuRecipes: Record<Recipe, number> }[])[0].menuRecipes;
      assertAndLog(
...
Deno.test("MenuCollectionConcept - createMenu Action", async (t) => {
  ...
  try {
    ...
    await t.step("1. Successfully create a menu", async () => {
      ...
      const queriedMenu = await menuCollection._getMenuDetails({ menu: menuId });
      assertAndLog(
...
Deno.test("MenuCollectionConcept - addRecipe Action", async (t) => {
  ...
  try {
    ...
    await t.step("1. Successfully add a recipe to a menu", async () => {
      ...
      const menuRecipes = (recipes as { menuRecipes: Record<Recipe, number> }[])[0].menuRecipes;
      assertAndLog(
    ...
    await t.step("3. Fail to add recipe with non-positive scaling factor", async () => {
      const currentStepMsg = "3. Fail to add recipe with non-positive scaling factor";
      printStepHeader(currentStepMsg);
      const addResult = await menuCollection.addRecipe({
        menu: menuId,
        recipe: RECIPE_VEGGIES,
    ...
    await t.step("4. Fail to add a recipe that already exists in the menu", async () => {
      const currentStepMsg =
...
Deno.test("MenuCollectionConcept - removeRecipe Action", async (t) => {
  ...
  try {
    ...
    await t.step("2. Fail to remove recipe from a non-existent menu", async () => {
      const currentStepMsg = "2. Fail to remove recipe from a non-existent menu";
      printStepHeader(currentStepMsg);
      const nonExistentMenu = freshID();
      const removeResult = await menuCollection.removeRecipe({
        menu: nonExistentMenu,
...
Deno.test("MenuCollectionConcept - changeRecipeScaling Action", async (t) => {
  ...
  try {
    ...
    await t.step("1. Successfully change recipe scaling factor", async () => {
      const currentStepMsg =
        "1. Successfully change recipe scaling factor";
      printStepHeader(currentStepMsg);
    ...
    await t.step("3. Fail to change scaling for recipe not in menu", async () => {
      const currentStepMsg = "3. Fail to change scaling for recipe not in menu";
      printStepHeader(currentStepMsg);
      const changeResult = await menuCollection.changeRecipeScaling({
        menu: menuId,
    ...
    await t.step("4. Fail to change scaling with non-positive new scaling factor", async () => {
      const currentStepMsg =
...
Deno.test("MenuCollectionConcept - Query Actions", async (t) => {
  ...
  try {
    ...
    await t.step("4. _getRecipesInMenu for empty menu", async () => {
      ...
      printStepHeader(currentStepMsg);
      const recipes = await menuCollection._getRecipesInMenu({ menu: bobMenuId });
      assertAndLog(
        "error" in recipes,
        false,
        "Should not return an error",
    ...
    await t.step("5. _getMenusOwnedByUser for user with multiple menus", async () => {
      const currentStepMsg =
    ...
    await t.step("6. _getMenusOwnedByUser for user with no menus", async () => {
      const currentStepMsg =
        "6. _getMenusOwnedByUser for user with no menus";
      printStepHeader(currentStepMsg);
      const userMenus = await menuCollection._getMenusOwnedByUser({
    ...
    await t.step("7. _getMenuByDate for existing menu/date/owner", async () => {
      const currentStepMsg =
        "7. _getMenuByDate for existing menu/date/owner";
      printStepHeader(currentStepMsg);
