...
content_id: 869cab3b7c72d258c779e3f70e46bd61d7b1ef775a0824e71b1460fd64cf536f
---
...
function getWeekStartAndEndDate(dateInWeekString: string): { startDate: string, endDate: string } {
...
export default class WeeklyCartConcept {
  ...
  async createCart({ dateInWeek, owner }: CreateCartInput): Promise<CreateCartOutput> {
    try {
      ...
      if (existingCart) {
        return { error: `A cart already exists for owner ${owner} for the week of ${dateInWeek}.` };
      }
  ...
  async deleteCart({ dateInWeek }: DeleteCartInput): Promise<DeleteCartOutput> {
    try {
      ...
      if (!cartToDelete) {
        return { error: `No cart found for the week containing ${dateInWeek}.` };
      }
  ...
  async addMenuToCart({ menu, menuDate }: AddMenuToCartInput): Promise<AddMenuToCartOutput> {
    try {
      ...
      if (!targetCart) {
        ...
        return { error: `No cart found for week of ${menuDate}. An owner is required to create a new cart.` };
      }
