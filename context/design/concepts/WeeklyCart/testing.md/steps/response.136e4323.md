---
timestamp: 'Mon Oct 27 2025 21:31:01 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251027_213101.ab45de2c.md]]'
content_id: 136e4323c7f290f5a27d6cc5baac475454e5265f72df635002cae5aded88da98
---
...
lastSunday.setUTCDate(today.getUTCDate() - (today.getUTCDay() === 0 ? 7 : today.getUTCDay()));
...
const twoWeeksFromNowCartId = twoWeeksFromSunday.toISOString().split("T")[0] as ID;
const lastWeekCartId = lastSunday.toISOString().split("T")[0] as ID;
...
Deno.test("WeeklyCartConcept - Operating Principle Verification", async (t) => {
  ...
  await t.step("1. User creates a cart for the week starting next Sunday", async () => {
    const stepMessage = "1. User creates a cart for the week starting next Sunday";
    printStepHeader(stepMessage);
    checkIndex = 0;

    const createResult = await weeklyCart.createCart({ dateInWeek: nextMonday });
    assertAndLog(
  ...
  await t.step("2. User adds several menus to the cart", async () => {
    ...
    const menusInCart = await weeklyCart._getMenusInCart({ cart: createdCartId });
    assertAndLog(
  ...
  await t.step("3. User removes a menu (Wednesday Pizza) from the cart", async () => {
    const stepMessage = "3. User removes a menu (Wednesday Pizza) from the cart";
    printStepHeader(stepMessage);
    checkIndex = 0;

    const removePizzaResult = await weeklyCart.removeMenuFromCart({ menu: menuPizza });
    assertAndLog(
    ...
    const menusInCartAfterRemoval = await weeklyCart._getMenusInCart({ cart: createdCartId });
    assertAndLog(
    ...
    assertAndLog(
      (menusInCartAfterRemoval as { menus: ID[] }[])[0].menus.includes(menuSpaghetti),
      true,
    ...
    assertAndLog(
      (menusInCartAfterRemoval as { menus: ID[] }[])[0].menus.includes(menuPizza),
      false,
    ...
    assertAndLog(
      (menusInCartAfterRemoval as { menus: ID[] }[])[0].menus.includes(menuTacos),
      true,
...
Deno.test("WeeklyCartConcept - createCart Action Tests", async (t) => {
  ...
  await t.step("1. Successfully create a cart for a future week", async () => {
    ...
    const createResult = await weeklyCart.createCart({ dateInWeek: nextWednesday });
    assertAndLog(
      "cart" in createResult,
      true,
      "Cart creation should succeed",
    ...
    assertAndLog(
      (queriedCartDates as { startDate: Date; endDate: Date }[])[0].startDate.getTime(),
      nextSunday.getTime(),
    ...
    assertAndLog(
      (queriedCartDates as { startDate: Date; endDate: Date }[])[0].endDate.getTime(),
  ...
  await t.step("2. Fail to create a cart for a week that has already started (past/current)", async () => {
    const stepMessage = "2. Fail to create a cart for a week that has already started";
    printStepHeader(stepMessage);
    checkIndex = 0;

    const createResult = await weeklyCart.createCart({ dateInWeek: lastMonday });
    assertAndLog(
    ...
    assertAndLog(
      (createResult as { error: string }).error.includes("already started or is in the past"),
      true,
  ...
  await t.step("3. Fail to create a duplicate cart for the same week", async () => {
    const stepMessage = "3. Fail to create a duplicate cart for the same week";
    printStepHeader(stepMessage);
    checkIndex = 0;
    ...
    const duplicateCreateResult = await weeklyCart.createCart({ dateInWeek: twoWeeksFromMonday });
    assertAndLog(
    ...
    assertAndLog(
      (duplicateCreateResult as { error: string }).error.includes("A cart already exists for the week starting on"),
      true,
...
Deno.test("WeeklyCartConcept - deleteCart Action Tests", async (t) => {
  ...
  await t.step("1. Successfully delete an existing cart", async () => {
    ...
    const deleteResult = await weeklyCart.deleteCart({ dateInWeek: nextWednesday });
    assertAndLog(
    ...
    const queriedCart = await weeklyCart._getCartByDate({ date: nextWednesday });
    assertAndLog(
      "error" in queriedCart,
      false,
  ...
  await t.step("2. Fail to delete a non-existent cart", async () => {
    ...
    const deleteResult = await weeklyCart.deleteCart({ dateInWeek: lastMonday });
    assertAndLog(
    ...
    assertAndLog(
      (deleteResult as { error: string }).error.includes("No cart found for the week containing"),
      true,
...
Deno.test("WeeklyCartConcept - addMenuToCart Action Tests", async (t) => {
  ...
  await t.step("1. Add menu to an existing cart successfully", async () => {
    ...
    const addMenuResult = await weeklyCart.addMenuToCart({ menu: menuSpaghetti, menuDate: nextMonday });
    assertAndLog(
    ...
    const menusInCart = await weeklyCart._getMenusInCart({ cart: existingCartId });
    assertAndLog(
      (menusInCart as { menus: ID[] }[])[0].menus.includes(menuSpaghetti),
      true,
  ...
  await t.step("2. Add menu to a week without a cart (should create new cart)", async () => {
    const stepMessage = "2. Add menu to a week without a cart (should create new cart)";
    printStepHeader(stepMessage);
    checkIndex = 0;

    const addMenuResult = await weeklyCart.addMenuToCart({ menu: menuPizza, menuDate: twoWeeksFromMonday });
    assertAndLog(
      "cart" in addMenuResult,
      true,
  ...
  await t.step("3. Fail to add a duplicate menu to the same cart", async () => {
    ...
    const duplicateAddResult = await weeklyCart.addMenuToCart({ menu: menuSpaghetti, menuDate: nextWednesday });
    assertAndLog(
    ...
    assertAndLog(
      (duplicateAddResult as { error: string }).error.includes("is already in the cart"),
      true,
  ...
  await t.step("4. Fail to add a menu for a past/current week if createCart fails", async () => {
    const stepMessage = "4. Fail to add a menu for a past/current week if createCart fails";
    printStepHeader(stepMessage);
    checkIndex = 0;

    const addMenuResult = await weeklyCart.addMenuToCart({ menu: menuSalad, menuDate: lastMonday });
    assertAndLog(
    ...
    assertAndLog(
      (addMenuResult as { error: string }).error.includes("Failed to create cart for menu addition"),
      true,
...
Deno.test("WeeklyCartConcept - removeMenuFromCart Action Tests", async (t) => {
  ...
  await t.step("1. Successfully remove a menu from a cart", async () => {
    ...
    const addSpaghettiResult = await weeklyCart.addMenuToCart({ menu: menuSpaghetti, menuDate: nextMonday });
    cartIdWithMenus = (addSpaghettiResult as { cart: ID }).cart;
    await weeklyCart.addMenuToCart({ menu: menuPizza, menuDate: nextWednesday });

    const removeResult = await weeklyCart.removeMenuFromCart({ menu: menuSpaghetti });
    assertAndLog(
    ...
    const menusInCart = await weeklyCart._getMenusInCart({ cart: cartIdWithMenus });
    assertAndLog(
      (menusInCart as { menus: ID[] }[])[0].menus.includes(menuSpaghetti),
      false,
  ...
  await t.step("2. Fail to remove a menu that is not in any cart", async () => {
    ...
    const removeResult = await weeklyCart.removeMenuFromCart({ menu: menuTacos }); // menuTacos not added yet
    assertAndLog(
    ...
    assertAndLog(
      (removeResult as { error: string }).error.includes("Menu not found in any cart"),
      true,
...
Deno.test("WeeklyCartConcept - Query Tests", async (t) => {
  ...
  await t.step("1. _getCartDates: Successfully retrieve cart dates", async () => {
    ...
    assertAndLog(
      (datesResult as { startDate: Date; endDate: Date }[])[0].startDate.getTime(),
      nextSunday.getTime(),
    ...
    assertAndLog(
      (datesResult as { startDate: Date; endDate: Date }[])[0].endDate.getTime(),
      nextSaturday.getTime(),
  ...
  await t.step("2. _getCartDates: Fail to retrieve dates for a non-existent cart", async () => {
    const stepMessage = "2. _getCartDates: Fail to retrieve dates for a non-existent cart";
    printStepHeader(stepMessage);
    checkIndex = 0;

    const datesResult = await weeklyCart._getCartDates({ cart: "nonExistentCart" as ID });
    assertAndLog(
      "error" in datesResult,
      true,
    ...
    assertAndLog(
      (datesResult as { error: string }).error.includes("Cart with ID nonExistentCart not found"),
      true,
  ...
  await t.step("3. _getMenusInCart: Successfully retrieve menus in cart", async () => {
    const stepMessage = "3. _getMenusInCart: Successfully retrieve menus in cart";
    printStepHeader(stepMessage);
    checkIndex = 0;

    await weeklyCart.addMenuToCart({ menu: menuSpaghetti, menuDate: nextWednesday });
    await weeklyCart.addMenuToCart({ menu: menuTacos, menuDate: nextWednesday });

    const menusResult = await weeklyCart._getMenusInCart({ cart: testCartId });
    assertAndLog(
  ...
  await t.step("4. _getMenusInCart: Fail to retrieve menus for a non-existent cart", async () => {
    const stepMessage = "4. _getMenusInCart: Fail to retrieve menus for a non-existent cart";
    printStepHeader(stepMessage);
    checkIndex = 0;

    const menusResult = await weeklyCart._getMenusInCart({ cart: "anotherNonExistentCart" as ID });
    assertAndLog(
      "error" in menusResult,
      true,
    ...
    assertAndLog(
      (menusResult as { error: string }).error.includes("Cart with ID anotherNonExistentCart not found"),
      true,
      "Error message should indicate cart not found",
      stepMessage,
      ++checkIndex,
  ...
  await t.step("5. _getCartByDate: Successfully retrieve cart by date", async () => {
    const stepMessage = "5. _getCartByDate: Successfully retrieve cart by date";
    printStepHeader(stepMessage);
    checkIndex = 0;

    const cartResult = await weeklyCart._getCartByDate({ date: nextWednesday });
    assertAndLog(
      "error" in cartResult,
      false,
  ...
  await t.step("6. _getCartByDate: Return empty array for no cart found", async () => {
    const stepMessage = "6. _getCartByDate: Return empty array for no cart found";
    printStepHeader(stepMessage);
    checkIndex = 0;
