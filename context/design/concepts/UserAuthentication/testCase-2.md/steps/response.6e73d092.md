---
timestamp: 'Wed Oct 15 2025 08:31:17 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251015_083117.484a95f2.md]]'
content_id: 6e73d092057457a9afefd75d0132ae0aa0d00e96be4667fdb8652b7f4fab6f15
---
...
Deno.test("UserAuthentication - Case 2 (Verify Password Changing)", async (t) => {
  ...
  const jackyPasswordInitial = "jacky_pass1";
  const jackyPasswordNew = "jacky_pass2";

  const henryUsername = "henry";
  const henryPasswordInitial = "henry_pass1";
  const henryPasswordNew = "henry_pass2";
  ...
  await t.step("1. Create first admin user Jacky", async () => {
    ...
    assertEquals(
      ...
      true,
      "Jacky should be able to authenticate with initial password",
    );
    assertEquals(
    ...
    assertEquals(
      ...
      true,
      "Jacky should be an admin (first user)",
    );
  });
  ...
  await t.step("2. Create second user Henry", async () => {
    ...
    assertEquals(
      ...
      true,
      "Henry should be able to authenticate with initial password",
    );
    assertEquals(
  ...
  await t.step("3. Verify Henry cannot change Jacky's password (incorrect old password)", async () => {
    // The concept's `updatePassword` method does not enforce external authorization (e.g., "calling user is admin").
    // It only checks if the `user` exists and if the `oldPassword` provided matches the stored password for that `user`.
    // Therefore, for Henry to "fail" to change Jacky's password, he must provide an incorrect `oldPassword`.
    const changeJackyPassByHenryResult = await userAuth.updatePassword({
      user: jackyUser,
      oldPassword: henryPasswordInitial, // Henry's password, which is incorrect for Jacky's account
      newPassword: "a_new_password_for_jacky_from_henry",
    });

    assertEquals(
      "error" in changeJackyPassByHenryResult,
      true,
      "Henry attempting to change Jacky's password with wrong oldPassword should fail",
    );
    assertEquals(
      (changeJackyPassByHenryResult as { error: string }).error,
      "Old password does not match.",
      "Error message should indicate old password mismatch",
    );

    // Verify Jacky can still authenticate with his original password, proving it wasn't changed
    const authJackyStillWorks = await userAuth.authenticate({
      username: jackyUsername,
      password: jackyPasswordInitial,
    });
    assertEquals(
      "user" in authJackyStillWorks,
      true,
  ...
  await t.step("4. Verify Jacky is able to change Jacky's password", async () => {
    const changeJackyPassResult = await userAuth.updatePassword({
      user: jackyUser,
      oldPassword: jackyPasswordInitial,
      newPassword: jackyPasswordNew,
    });
    assertEquals(
      "success" in changeJackyPassResult,
      true,
      "Jacky changing his own password should succeed",
    );
    assertEquals(
      (changeJackyPassResult as { success: true }).success,
      true,
      "Jacky's password change operation should return success",
    );

    // Verify authentication with old password fails
    const authJackyOldPass = await userAuth.authenticate({
      username: jackyUsername,
      password: jackyPasswordInitial,
    });
    assertEquals(
      "error" in authJackyOldPass,
      true,
      "Jacky should no longer be able to authenticate with old password",
    );
    assertEquals(
      (authJackyOldPass as { error: string }).error,
      "Invalid username or password.",
      "Authentication with old password should fail with correct error",
    );

    // Verify authentication with new password succeeds
    const authJackyNewPass = await userAuth.authenticate({
      username: jackyUsername,
      password: jackyPasswordNew,
    });
    assertEquals(
      "user" in authJackyNewPass,
      true,
      "Jacky should be able to authenticate with new password",
    );
    assertEquals(
      (authJackyNewPass as { user: ID }).user,
      jackyUser,
  ...
  await t.step("5. Verify Jacky is able to change Henry's password", async () => {
    // The concept's `updatePassword` method does not enforce external authorization (e.g., "calling user is admin").
    // Jacky, being an admin, would typically be authorized by an external layer to initiate this action for Henry.
    // Here, we're testing the concept's action itself, assuming the caller has the necessary information.
    const changeHenryPassResult = await userAuth.updatePassword({
      user: henryUser,
      oldPassword: henryPasswordInitial,
      newPassword: henryPasswordNew,
    });
    assertEquals(
      "success" in changeHenryPassResult,
      true,
      "Jacky changing Henry's password should succeed",
    );
    assertEquals(
      (changeHenryPassResult as { success: true }).success,
      true,
      "Henry's password change operation should return success",
    );

    // Verify authentication with Henry's old password fails
    const authHenryOldPass = await userAuth.authenticate({
      username: henryUsername,
      password: henryPasswordInitial,
    });
    assertEquals(
      "error" in authHenryOldPass,
      true,
      "Henry should no longer be able to authenticate with old password",
    );
    assertEquals(
      (authHenryOldPass as { error: string }).error,
      "Invalid username or password.",
      "Authentication with Henry's old password should fail with correct error",
    );

    // Verify authentication with Henry's new password succeeds
    const authHenryNewPass = await userAuth.authenticate({
      username: henryUsername,
      password: henryPasswordNew,
    });
    assertEquals(
      "user" in authHenryNewPass,
      true,
      "Henry should be able to authenticate with new password",
    );
    assertEquals(
      (authHenryNewPass as { user: ID }).user,
      henryUser,
