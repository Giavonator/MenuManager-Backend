---
timestamp: 'Thu Oct 23 2025 19:00:44 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251023_190044.cabb44ce.md]]'
content_id: bd21f85a84b9178603a1a302ce51955f5cffb68eb0b9277bbbf495b10fdb6504
---
...
// updatePurchaseOption (overloaded variants)
...
// Additional actions found in implementation, not explicitly in spec provided
...
// (Queries return Array<SuccessType> on success, or { error: string } on failure)
// -----------------------------------------------------------------------------

// _getItemByName (name: String): (item: Item)
type GetItemByNameInput = { name: string };
type GetItemByNameOutput = Array<{ item: Item }> | { error: string };
...
type GetItemByPurchaseOptionOutput = Array<{ item: Item }> | { error: string };

// _getItemNames (item: Item): (names: Set of String)
type GetItemNamesInput = { item: Item };
type GetItemNamesOutput = Array<{ names: string[] }> | { error: string };
...
type GetAllItemsInput = Empty; // No specific input arguments
type GetAllItemsOutput = Array<{ items: Item[] }>; // Assumes no explicit error object for this query as per spec. It returns an empty array if no items.
...
export default class StoreCatalogConcept {
  ...
  async _getItemByName(
    ...
    if (!itemDoc) {
      // Per instructions, query errors return a single object {error: string}
