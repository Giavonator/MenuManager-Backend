---
timestamp: 'Wed Oct 15 2025 19:33:45 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251015_193345.49e494be.md]]'
content_id: 2e9c4535bc21b3d20fbd449390b3f8d4ed396b6bb0f1a2b841d9ef66e3d6e390
---

# response:

```typescript
// file: src/MenuManager/MenuManagerConcept.ts

import { Collection, Db, MongoClient } from "npm:mongodb";
import { ID, Empty } from "@utils/types.ts";
import { freshID } from "@utils/database.ts";
import { GeminiLLM } from "../utils/gemini-llm.ts"; // Assuming gemini-llm.ts is in ../utils
...
export default class MenuManagerConcept {
  ...
  private async calculateDishPrice(recipe: RecipeDoc): Promise<number> {
    ...
    for (const ingredient of recipe.ingredients) {
      ...
      if (itemDoc && itemDoc.quantity > 0) {
        ...
        console.warn(`Ingredient item ${ingredient.itemId} not found for recipe ${recipe._id}`);
      }
    }
  ...
  private async calculateMenuCost(menu: MenuDoc): Promise<number> {
    const recipesInMenu = await this.recipes.find({ menuId: menu._id }).toArray();
    let totalCost = 0;
  ...
  private async updateMenuCost(menuId: Menu): Promise<void> {
    ...
    if (menu) {
      ...
      await this.menus.updateOne({ _id: menuId }, { $set: { menuCost: newCost } });
    }
  }
  ...
  private async updateCartWeeklyCost(cartId: Cart): Promise<void> {
    ...
    if (cart) {
      ...
      await this.carts.updateOne({ _id: cartId }, { $set: { weeklyCost: newCost } });
    }
  }
  ...
  private async recalculateAllCostsForItem(itemId: Item): Promise<void> {
    const recipesUsingItem = await this.recipes.find({ "ingredients.itemId": itemId }).toArray();
    for (const recipe of recipesUsingItem) {
      ...
      const cartsContainingMenu = await this.carts.find({ menuIds: recipe.menuId }).toArray();
  ...
  async pullRecipeFromWebsite(
    { menu: menuId, recipeURL, owner }: { menu: Menu; recipeURL: string; owner: User },
  ): Promise<{ recipe: Recipe } | { error: string }> {
    const menu = await this.menus.findOne({ _id: menuId, owner });
    if (!menu) {
      return { error: `Menu with ID "${menuId}" not found or not owned by user.` };
    }
    ...
    try {
      ...
      const mentionedPercentage = (parsed.ingredients.length > 0) ? (mentionedIngredients / parsed.ingredients.length) * 100 : 100;
      if (parsed.ingredients.length > 0 && mentionedPercentage < 75) {
        console.warn(`⚠️ Semantic Validation Failed: Only ${mentionedPercentage.toFixed(0)}% of ingredients are mentioned in the instructions. This might indicate a poor parse or mismatched data.`);
        // For now, allow it but log a warning. Could be a hard error depending on business logic.
      } else if (parsed.ingredients.length > 0) {
        console.log(`✅ Instructions: ${mentionedPercentage.toFixed(0)}% of ingredients are mentioned, likely matching.`);
      }
      ...
      for (const ing of parsed.ingredients) {
        ...
        if (!itemDoc) {
          ...
          if ('error' in placeholderItemResult) {
            console.warn(`Could not create placeholder item "${ing.name}": ${placeholderItemResult.error}. Skipping this ingredient.`);
            continue;
          }
          itemDoc = await this.items.findOne({ _id: placeholderItemResult.item });
          if (!itemDoc) {
            console.error(`Failed to retrieve newly created placeholder item ${placeholderItemResult.item}. This should not happen.`);
            continue;
          }
        }
      ...
      const updatedRecipeDoc = { ...newRecipeDoc, ingredients: recipeIngredients }; // Temporary for calculation
      updatedRecipeDoc.dishPrice = await this.calculateDishPrice(updatedRecipeDoc);
      await this.recipes.updateOne({ _id: newRecipeId }, { $set: { dishPrice: updatedRecipeDoc.dishPrice } });
      await this.updateMenuCost(menuId);

      // Also update cart cost if this menu is in any cart
      const cartsContainingMenu = await this.carts.find({ menuIds: menuId }).toArray();
      for (const cart of cartsContainingMenu) {
        await this.updateCartWeeklyCost(cart._id);
      }

      console.log(`✅ Successfully parsed and stored recipe: "${updatedRecipeDoc.name}"`);
      return { recipe: newRecipeId };

    } catch (parseError) {
      console.error('❌ Error parsing LLM response or adding recipe:', (parseError as Error).message);
      return { error: `Failed to process LLM response: ${(parseError as Error).message}` };
    }
  }
   ...
   */
  async createRecipe(
    { menu: menuId, name, instructions, servingQuantity, dishType, scalingFactor, owner }: {
      menu: Menu;
      name: string;
      instructions: string;
      servingQuantity: number;
  ...
  async updateRecipeIngredient(
    ...
    if (!recipe) {
      return { error: `Recipe with ID "${recipeId}" not found in menu "${menuId}".` };
    }
    if (amount < 0) {
    ...
    await this.recipes.updateOne({ _id: recipeId }, { $set: { ingredients: recipe.ingredients } });

    // Recalculate dish price
    recipe.dishPrice = await this.calculateDishPrice(recipe);
    await this.recipes.updateOne({ _id: recipeId }, { $set: { dishPrice: recipe.dishPrice } });
  ...
  async updateRecipeServingQuantity(
    ...
    await this.recipes.updateOne({ _id: recipeId }, { $set: { servingQuantity } });
  ...
  async updateRecipeScalingFactor(
    ...
    await this.recipes.updateOne({ _id: recipeId }, { $set: { scalingFactor } });
    ...
    await this.recipes.updateOne({ _id: recipeId }, { $set: { dishPrice: updatedRecipe.dishPrice } });
  ...
  async updateRecipeName(
    { menu: menuId, recipe: recipeId, name }: { menu: Menu; recipe: Recipe; name: string },
  ): Promise<Empty | { error: string }> {
    const recipe = await this.recipes.findOne({ _id: recipeId, menuId });
    if (!recipe) {
      return { error: `Recipe with ID "${recipeId}" not found in menu "${menuId}".` };
  ...
  async enterItem(
    ...
    if (existingItem) {
      return { error: `An item already exists with the name "${name}". Use addItemName to add an alias.` };
    }
    if (price < 0) {
  ...
  async addItemName(
    ...
    const existingItemWithSameName = await this.items.findOne({ _id: { $ne: itemId }, names: name });
  ...
  async addMenuToCart(
    ...
    const menusInCart = await this.menus.find({ _id: { $in: cart.menuIds } }).toArray();
    if (menusInCart.some((m) => m.date === menu.date)) {
      return { error: `Cart "${cartId}" already contains a menu for date "${menu.date}".` };
    }
  ...
  async adjustRecipeScale(
    ...
    if (!cart) {
      return { error: `Cart "${cartId}" does not exist or does not contain menu "${menuId}".` };
    }
  ...
  async adjustItemQuantity(
    ...
    if (recipesUsingItemInMenu === 0) {
      return { error: `Item "${itemId}" is not used in any recipe within menu "${menuId}".` };
    }
    ...
    const updateResult = await this.items.updateOne({ _id: itemId }, { $set: { quantity } });
   ...
   */
  async _getMenusInCart({ cart: cartId }: { cart: Cart }): Promise<{ menus: MenuDoc[] } | { error: string }> {
    const cart = await this.carts.findOne({ _id: cartId });
    if (!cart) {
      return { error: `Cart with ID "${cartId}" not found.` };
    }
    const menus = await this.menus.find({ _id: { $in: cart.menuIds } }).toArray();
    return { menus };
  }
   ...
   */
  async _getRecipesInMenu({ menu: menuId }: { menu: Menu }): Promise<{ recipes: RecipeDoc[] } | { error: string }> {
    const menu = await this.menus.findOne({ _id: menuId });
    if (!menu) {
      return { error: `Menu with ID "${menuId}" not found.` };
    }
  ...
  ): Promise<{ ingredients: Record<Item, { amount: number; units: string }> } | { error: string }> {
  ...
  async _getIngredientsInMenu(
    ...
    const aggregatedIngredients: Record<Item, { amount: number; units: string }> = {};
      ...
      for (const ing of recipe.ingredients) {
        ...
        if (item) {
          ...
          if (aggregatedIngredients[item._id]) {
            ...
            aggregatedIngredients[item._id] = { amount: scaledAmount, units: item.units };
          }
        } else {
          console.warn(`Item ${ing.itemId} not found for recipe ${recipe._id} in menu ${menuId}`);
        }
      }
    }
  ...
  async _getIngredientsPerStore(
  ...
  ): Promise<{ storeShoppingList: Record<string, Record<Item, { amount: number; units: string }>> }> {
    const storeShoppingList: Record<string, Record<Item, { amount: number; units: string }>> = {};
    ...
    for (const menuId of menuIds) {
      ...
      const menuIngredientsResult = await this._getIngredientsInMenu({ menu: menuId });
      if ('error' in menuIngredientsResult) {
        console.warn(`Error getting ingredients for menu ${menuId}: ${menuIngredientsResult.error}`);
        continue;
      }
   ...
   */
  async _getMenuByDate({ date }: { date: string }): Promise<{ menu: MenuDoc } | { error: string }> {
   ...
   */
  async _getMenusOwnedByUser({ user: ownerId }: { user: User }): Promise<{ menus: MenuDoc[] }> {
