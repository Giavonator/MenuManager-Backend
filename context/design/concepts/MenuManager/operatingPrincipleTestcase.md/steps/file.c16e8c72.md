...
content_id: c16e8c72cdb3765c2231d76d3c0aea1fa6e895cebcf8b3894936c51cfb4005b5
---
...
export default class MenuManagerConcept {
  ...
  async _getIngredientsPerStore(
    ...
    for (const menuId of menuIds) {
      ...
      // FIX START: Iterate over Object.keys to correctly type itemId
      for (const key of Object.keys(menuIngredients)) {
        const itemId = key as Item; // Explicitly cast the string key to Item (ID)
...
Please fix \_getIngredientsPerStore it has this issue:

Element implicitly has an 'any' type because expression of type 'string' can't be used to index type 'Record\<ID, { amount: number; units: string; }>'.
No index signature with a parameter of type 'string' was found on type 'Record\<ID, { amount: number; units: string; }>'. Line 1433 Col 35.
