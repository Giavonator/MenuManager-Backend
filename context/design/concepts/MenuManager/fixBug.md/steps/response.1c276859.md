---
timestamp: 'Thu Oct 16 2025 15:55:05 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251016_155505.db32a131.md]]'
content_id: 1c27685942c8e70b7570f6c7c30b95546798826540aaa48b2cc52403b8c59f55
---
...
Deno.test("MenuManagerConcept - Operating Principle", async (t) => {
  ...
  // --- Sub-step 3.1: Use LLM to pull in recipe from website (initial parse validation) ---
  await t.step(
    printStepHeader(
      "Part 3.1: Use LLM to pull in recipe from website (initial parse validation)",
    ),
    async () => {
      ...
      // Verify basic fields exist and are of expected type, without assuming specific content from the parse
      ...
      assertAndLog(
        llmRecipe!.servingQuantity >= 0, // Allow 0, though positive is usually expected
        true,
        "LLM-pulled recipe serving quantity should be non-negative",
      );
      ...
      if (llmRecipeIngredients.length > 0) {
        ...
        for (const ing of llmRecipeIngredients) {
          ...
          assertAndLog(
            typeof ing.amount,
            "number",
            `Ingredient ${itemName} amount should be a number`,
          );
          assertAndLog(
            ing.amount >= 0,
            true,
            `Ingredient ${itemName} amount should be non-negative`,
          );
          console.log(
  ...
  // --- Sub-step 3.2: Update LLM-pulled recipe attributes and ingredients ---
  await t.step(
    printStepHeader(
      "Part 3.2: Update LLM-pulled recipe attributes and ingredients",
    ),
    async () => {
  ...
  await t.step(printStepHeader("Part 4.1: Confirm items"), async () => {
    ...
    // With the current logic, `pullRecipeFromWebsite` only adds ingredients that are already
    // registered in the system. Thus, `_getListOfItems` will already contain them.
    // This loop primarily confirms items manually entered (which are stored in itemDetails)
    // or any other existing items that might not have been confirmed yet.
    ...
    for (const itemId of allItemIds) {
      ...
      if (!initialItem) {
        console.warn(
          `Item ${itemId} not found in DB during confirmation. This should not happen if all items were registered correctly.`,
        );
        continue;
      }
