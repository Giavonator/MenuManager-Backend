---
timestamp: 'Thu Oct 16 2025 12:03:30 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251016_120330.049cafc5.md]]'
content_id: f89d8600949c1a864563a8c6771f4b0db6e3c4f9e2b57624474a1901f451db8c
---

# response:

The error "Started test step while another test step with sanitizers was running" in Deno tests most commonly occurs when there are un-awaited asynchronous operations within a test step, or when Promises are implicitly created but not handled.

Upon review, a critical issue was identified in how `assertAndLog` (and some `console.log` statements) were used. The `message` argument passed to `assertEquals` (and similarly, the string for `console.log`) was constructed using **`await` inside a template literal**.
...
assertAndLog(
  ...,
  `Add ingredient ${
    (await db.collection("MenuManager.items").findOne({ _id: ing.itemId }))?.names[0] || ing.itemId
...
1. The `assertEquals` function expects a `string` for its message argument, not a `Promise<string>` or a value that is still being asynchronously resolved.
2. When `await` is used within a template literal in an argument list, the argument might resolve *after* the outer function (`assertAndLog` in this case) has already received it or moved on, effectively creating an un-awaited promise that Deno's test runner can detect as a "leaked" asynchronous operation. This can conflict with Deno's resource sanitizers, leading to the reported error.

The fix involves extracting all `await` calls that are used to construct the `message` string *before* calling `assertAndLog` or `console.log`.

**Here's the detailed fix:**

1. **Introduce an `getItemName` helper function:** This function will asynchronously fetch the item name from the database and return it, centralizing the logic and making the test code cleaner.
2. **Pre-resolve item names:** Before calling `assertAndLog` or `console.log` in any place where an item name is dynamically fetched, use the new `getItemName` helper to get the name and then pass the resolved string to the logging/assertion function.

This ensures that all asynchronous database lookups for item names are fully awaited and completed *before* the message string is constructed and passed to `assertAndLog`, preventing un-awaited promises from interfering with Deno's test runner and sanitizers.
...
function assertAndLog<T>(
  ...
  try {
    // BUG FIX NOTE (Line 638):
    // The error "Expected 1-2 arguments, but got 4. Line 638" for `assertEquals(actual, expected, message)`
    // is highly anomalous. The `assertEquals` function from `jsr:@std/assert` correctly supports 2 or 3 arguments (actual, expected, optional_message).
    // This line is syntactically correct for the specified library. It's likely an environmental
    // or linter misconfiguration, or a misleading error report. No code change is applied here
    // as the line conforms to the standard library's API.
...
Deno.test("MenuManagerConcept - Full Lifecycle Test", async (t) => {
  ...
  const getItemName = async (itemId: ID): Promise<string> => {
    const item = await db.collection("MenuManager.items").findOne({ _id: itemId });
  ...
  await t.step("Part 3: LLM Recipe Pull and Update", async () => {
    printStepHeader("Part 3: LLM Recipe Pull and Update");
    let checkIndex = 0; // Reset checkIndex for this major step
  ...
  await t.step("Part 4: Item Confirmation and Cart Management", async () => {
    printStepHeader("Part 4: Item Confirmation and Cart Management");
    let checkIndex = 0; // Reset checkIndex for this major step
