---
timestamp: 'Thu Oct 16 2025 15:38:33 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251016_153833.cbda5995.md]]'
content_id: c82001a5088f1afca63e074c48ccc8710175f68271886fc1c41a93596a2c4108
---
...
Deno.test("MenuManagerConcept - Operating Principle", async (t) => {
  ...
  // let expectedLlmInitialCost: number = 0; // Removed: No longer assuming specific LLM initial parse values
  ...
  // Declare target ingredients for Part 3.2/3.3 here so it's accessible to both
  ...
  await t.step(
    ...
    async () => {
      ...
      console.log(
        `🤖 LLM-parsed initial ingredients (only pre-existing items added to DB, if any):`,
      );

      const llmRecipeIngredients = llmRecipe?.ingredients || [];
      let llmActualInitialRecipeCost = 0; // Dynamically calculate based on what LLM *actually* parsed

      for (const ing of llmRecipeIngredients) {
        const itemName = await getItemName(ing.itemId);
        // Using itemDetails for price/quantity (assumed to be correct reference data)
        const itemInfo = itemDetails[ing.itemId];
        if (itemInfo && itemInfo.quantity > 0) {
          // LLM-parsed `amount` is directly used here, it's NOT converted to itemDoc.units at this stage
          // The calculateDishPrice in the concept uses the raw LLM amount against the ItemDoc's base units.
          llmActualInitialRecipeCost += (itemInfo.price / itemInfo.quantity) *
            ing.amount;
        } else {
          console.warn(
            `Item ${ing.itemId} (${itemName}) not found in itemDetails or has zero quantity for initial LLM cost calculation. Assuming 0 cost.`,
          );
        }
        console.log(`   - ${itemName}: ${ing.amount}`);
      }
      llmActualInitialRecipeCost = parseFloat(
        llmActualInitialRecipeCost.toFixed(2),
      );

      // Verify LLM recipe dishPrice. It should accurately reflect the cost of *whatever* LLM parsed.
      assertAndLog(
        llmRecipe?.dishPrice,
        llmActualInitialRecipeCost,
        "LLM-pulled recipe dishPrice should be correct based on actual LLM parsed amounts",
      );
      ...
      const expectedMenuCostAfterLlmPull = parseFloat(
        (expectedPancakesCost + expectedVanillaCost + llmActualInitialRecipeCost)
          .toFixed(2),
      );
      assertAndLog(
  ...
  // --- Sub-step 3.2: Update LLM-pulled recipe to match Recipe #2 ---
  await t.step(
    ...
    async () => {
      ...
      // Fetch current ingredients parsed by LLM (which could be anything initially)
  ...
  // --- Sub-step 3.3: Verify LLM-pulled recipe after correction ---
  await t.step(
    printStepHeader("Part 3.3: Verify LLM-pulled recipe after correction"),
    async () => {
