---
timestamp: 'Thu Oct 16 2025 14:44:20 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251016_144420.4e0a4f33.md]]'
content_id: 5fc046f94fa7de9fb7dbe880d9c754bc6c4fc3d4165a0315bbb8c1f8d0b5d09a
---
...
// Global variables to manage step-specific context for logging
let globalStepMessage: string = "";
let globalCheckIndex: number = 0;

function assertAndLog<T>(
  actual: T,
  expected: T,
  message: string,
  // Removed stepMessage and checkIndex parameters, now using globals
) {
  globalCheckIndex++; // Increment the global check index for each assertion
  try {
...
// MODIFIED: This function now sets globalStepMessage and resets globalCheckIndex.
// This allows it to be used directly as the first argument to Deno.test's `t.step`,
// fulfilling the "reduction" request while also printing the header.
function printStepHeader(stepMessage: string): string {
  console.log(`\n### ➡️  Step: ${stepMessage}\n`);
  globalStepMessage = stepMessage; // Set the global step message
  globalCheckIndex = 0; // Reset check index for each new step
...
Deno.test("MenuManagerConcept - Operating Principle", async (t) => {
  ...
  await t.step(
    ...
    async () => {
      // Removed: const stepMessage = "Part 1: User creates an empty menu";
      // Removed: let checkIndex = 0;
      ...
      assertAndLog(
        ...
        // Removed stepMessage, ++checkIndex
      );
  ...
  await t.step(printStepHeader("Part 2.1: Register global items"), async () => {
    // Removed: const stepMessage = "Part 2.1: Register global items";
    // Removed: let checkIndex = 0;
  ...
  await t.step(
    ...
    async () => {
      // Removed: const stepMessage = "Part 2.2: Create Classic Pancakes recipe";
      // Removed: let checkIndex = 0;
  ...
  await t.step(
    ...
    async () => {
      // Removed: const stepMessage = "Part 2.3: Create Vanilla Cupcakes recipe (manual)";
      // Removed: let checkIndex = 0;
  ...
  await t.step(
    ...
    async () => {
      // Removed: const stepMessage = "Part 3.1: Use LLM to pull in recipe from website";
      // Removed: let checkIndex = 0;
      ...
      if ("error" in pullRecipeResult) {
        ...
        // Removed the failing assertion: assertAndLog("recipe" in pullRecipeResult, true, "...", ++checkIndex);
        // Throwing an error here prevents subsequent checks from running if the LLM pull truly failed critically.
        throw new Error(
  ...
  await t.step(
    ...
    async () => {
      // Removed: const stepMessage = "Part 3.2: Update LLM-pulled recipe to match Recipe #2";
      // Removed: let checkIndex = 0;
  ...
  await t.step(printStepHeader("Part 4.1: Confirm items"), async () => {
    // Removed: const stepMessage = "Part 4.1: Confirm items";
    // Removed: let checkIndex = 0;
  ...
  await t.step(printStepHeader("Part 4.2: Create a cart"), async () => {
    // Removed: const stepMessage = "Part 4.2: Create a cart";
    // Removed: let checkIndex = 0;
  ...
  await t.step(printStepHeader("Part 4.3: Add menu to the cart"), async () => {
    // Removed: const stepMessage = "Part 4.3: Add menu to the cart";
    // Removed: let checkIndex = 0;
  ...
  await t.step(
    ...
    async () => {
      // Removed: const stepMessage = "Part 4.4: Administrator updates item price, verify cost recalculation";
      // Removed: let checkIndex = 0;
