...
content_id: 07cf72d0f26fe1fc8f320501bcbc0c3ca56c8dc366d7716de9c92ec6f4597d48
---
...
Deno.test("MenuManagerConcept - Operating Principle", async (t) => {
  ...
  await t.step("Part 2.1: Register global items", async () => {
    ...
    assertAndLog(
      !("error" in (flourResult as { item: ID } | { error: string })), // Updated check // Check if it's NOT an error object
      true,
    ...
    assertAndLog(
      !("error" in (milkResult as { item: ID } | { error: string })), // Updated check
      true,
    ...
    assertAndLog(
      !("error" in (eggResult as { item: ID } | { error: string })), // Updated check
      true,
    ...
    assertAndLog(
      !("error" in (butterResult as { item: ID } | { error: string })), // Updated check
      true,
    ...
    assertAndLog(
      !("error" in (sugarResult as { item: ID } | { error: string })), // Updated check
      true,
    ...
    assertAndLog(
      !("error" in (bakingPowderResult as { item: ID } | { error: string })), // Updated check
      true,
    ...
    assertAndLog(
      !("error" in (waterResult as { item: ID } | { error: string })), // Updated check
      true,
