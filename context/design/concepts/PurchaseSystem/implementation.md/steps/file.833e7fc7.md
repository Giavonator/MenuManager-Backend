---
timestamp: 'Tue Oct 28 2025 19:00:14 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251028_190014.c961a3e5.md]]'
content_id: 833e7fc7b7d30c8ddbda7ea5f2e385536fa78cd17d06cc0b19dade334ec4f840
---
...
export default class PurchaseSystemConcept {
  ...
  async purchaseOrder(input: PurchaseOrderInput): Promise<PurchaseOrderOutput> {
    try {
      ...
      // Requirement Check: All `SelectOrder`s within the tree have at least one AtomicOrder option.
      ...
      while (compositeOrdersQueue.length > 0) {
        ...
        const currentCompositeDoc = await this.compositeOrders.findOne({
          _id: currentCompositeID,
        });
        ...
        if (!currentCompositeDoc) {
          return {
            error:
              `Internal error: CompositeOrder '${currentCompositeID}' not found during purchase validation.`,
          };
        }

        // Check child SelectOrders of the current composite order
        for (
          const selectID of Object.keys(currentCompositeDoc.childSelectOrders)
        ) {
          ...
          if (!selectOrderDoc || selectOrderDoc.baseQuantity === -1) {
            return {
              error:
                `Purchase cannot be completed: SelectOrder '${typedSelectID}' (associateID: ${
                  selectOrderDoc?.associateID || "N/A"
                }) has no valid AtomicOrder options.`,
            };
          }
        }
        ...
        for (
          const childCompID of Object.keys(
            currentCompositeDoc.childCompositeOrders,
          )
        ) {
