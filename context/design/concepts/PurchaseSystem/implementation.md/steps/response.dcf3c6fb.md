---
timestamp: 'Tue Oct 28 2025 16:01:32 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251028_160132.97aa6f5a.md]]'
content_id: dcf3c6fbf2e2129835bd661866686a404a8a1cb8e3c4efad464aa2ad5a8f0940
---
...
// Generic types of this concept (ID is already defined in @utils/types.ts)
type EntityID = ID; // Using EntityID to avoid confusion with the ID type from @utils/types.ts for concept-specific generic.
...
// createSelectOrder
type CreateSelectOrderInput = { associateID: EntityID };
...
type CreateCompositeOrderInput = { associateID: EntityID };
...
type GetOrderByAssociateIDInput = { associateID: EntityID };
...
type GetOptimalPurchaseOutput = Result<{ optimalPurchase: Record<ID, number> }[]>;
...
export default class PurchaseSystemConcept {
  ...
  async createSelectOrder(
    ...
    try {
      ...
      if (await this.findAnyOrderByAssociateID(associateID)) {
        return { error: `Order with associateID '${associateID}' already exists.` };
      }
      ...
      return { error: `Failed to create SelectOrder: ${this.getErrorMessage(e)}` };
    }
  }
  ...
  async createAtomicOrder(
    ...
    try {
      const { selectOrder: selectOrderID, associateID, quantity, units, price } =
        input;
      ...
      await this.selectOrders.updateOne({ _id: selectOrderID }, updateSelectOrder);
      ...
      return { error: `Failed to create AtomicOrder: ${this.getErrorMessage(e)}` };
    }
  }
  ...
  async deleteAtomicOrder(
    ...
    try {
      ...
      if (!existingSelectOrder.childAtomicOrders.includes(atomicOrderID)) {
        return {
          error: `AtomicOrder '${atomicOrderID}' is not a child of SelectOrder '${selectOrderID}'.`,
        };
      }
      ...
      return { error: `Failed to delete AtomicOrder: ${this.getErrorMessage(e)}` };
    }
  }
  ...
  async addSelectOrderToCompositeOrder(
    ...
    try {
      const { compositeOrder: compositeOrderID, selectOrder: selectOrderID, scaleFactor } =
        input;
      ...
      return {
        error: `Failed to add SelectOrder to CompositeOrder: ${this.getErrorMessage(e)}`,
      };
    }
  }
  ...
  async removeSelectOrderFromCompositeOrder(
    ...
    try {
      const { compositeOrder: compositeOrderID, selectOrder: selectOrderID } = input;
      ...
      if (!(selectOrderID in existingCompositeOrder.childSelectOrders)) {
        return {
          error: `SelectOrder '${selectOrderID}' is not a child of CompositeOrder '${compositeOrderID}'.`,
        };
      }
      ...
      return {
        error: `Failed to remove SelectOrder from CompositeOrder: ${this.getErrorMessage(e)}`,
      };
    }
  }
  ...
  async addCompositeSubOrder(
    ...
    try {
      ...
      if (await this.wouldFormCycle(parentOrderID, childOrderID)) {
        return {
          error: `Adding CompositeOrder '${childOrderID}' to '${parentOrderID}' would form a cycle.`,
        };
      }
  ...
  async removeCompositeSubOrder(
    ...
    try {
      ...
      if (!(childOrderID in existingParentOrder.childCompositeOrders)) {
        return {
          error: `Child CompositeOrder '${childOrderID}' is not a child of Parent CompositeOrder '${parentOrderID}'.`,
        };
      }
      ...
      return {
        error: `Failed to remove Composite sub-order: ${this.getErrorMessage(e)}`,
      };
    }
  }
  ...
  async updateSubOrderScaleFactor(
    ...
    try {
      const { parentOrder: parentOrderID, childOrder: childOrderID, newScaleFactor } =
        input;
      ...
      if (childOrderID in existingParentOrder.childSelectOrders) {
        ...
        return {
          error: `Child Order '${childOrderID}' is not a direct child of CompositeOrder '${parentOrderID}'.`,
        };
      }
      ...
      return {
        error: `Failed to update sub-order scale factor: ${this.getErrorMessage(e)}`,
      };
    }
  }
  ...
  async deleteCompositeOrder(
    ...
    try {
      ...
      for (const selectOrderID of Object.keys(
      ...
      const childCompositeOrderIDs = Object.keys(existingCompositeOrder.childCompositeOrders);
      ...
      if (parentOfDeleted !== compositeOrderID) { // If it had a parent, recalculate its parent's root
        const parentOrderDoc = await this.compositeOrders.findOne({_id: parentOfDeleted});
        if (parentOrderDoc) rootOrdersToRecalculate.add(parentOrderDoc.rootOrder);
      } else {
      ...
      if (rootOrdersToRecalculate.size > 0) {
        await this.calculateOptimalPurchase({ compositeOrders: Array.from(rootOrdersToRecalculate) });
      }
  ...
  async calculateOptimalPurchase(
    ...
    try {
      ...
      for (const compositeOrderID of compositeOrderIDs) {
        ...
        const calculatedCompositeOrders = new Map<ID, { cost: number; optimal: Record<ID, number> }>();

        while (compositeOrderProcessQueue.length > 0) {
            const currentCompositeID = compositeOrderProcessQueue.shift()!;
            const currentComposite = allCompositeOrdersInTree.get(currentCompositeID);
            if (!currentComposite) continue;

            let currentCompositeTotalCost = 0;
            const currentCompositeOptimalPurchase: Record<ID, number> = {};
            const localAggregatedSelectOrderRequirements: Map<ID, { quantity: number; units: string }> = new Map();
            ...
            for (const selectID in currentComposite.childSelectOrders) {
                ...
                const selectOrderDoc = await this.selectOrders.findOne({ _id: selectID as ID });
                if (!selectOrderDoc || selectOrderDoc.baseQuantity === -1) {
                    continue; // SelectOrder without atomic options or uninitialized
                }
                const totalRequiredQuantity = selectScale * selectOrderDoc.baseQuantity;
                const existingRequirement = localAggregatedSelectOrderRequirements.get(selectID as ID);
                if (existingRequirement) {
                    existingRequirement.quantity += totalRequiredQuantity;
                    localAggregatedSelectOrderRequirements.set(selectID as ID, existingRequirement);
                } else {
            ...
            for (const [selectOrderID, required] of localAggregatedSelectOrderRequirements.entries()) {
                ...
                for (const atomicOption of atomicOptions) {
                    ...
                    if (atomicOption.units !== required.units) {
                        console.warn(`Unit mismatch for SelectOrder '${selectOrderID}' (req: ${required.units}) and AtomicOrder '${atomicOption._id}' (prov: ${atomicOption.units}). This may lead to inaccurate cost calculations.`);
                        continue;
                    }
                ...
                if (bestAtomicOption) {
                    currentCompositeOptimalPurchase[bestAtomicOption._id] = (currentCompositeOptimalPurchase[bestAtomicOption._id] || 0) + optimalUnitsToBuy;
            ...
            for (const childCompID in currentComposite.childCompositeOrders) {
                ...
                const childCalculated = calculatedCompositeOrders.get(childCompID as ID);
                if (childCalculated) {
                    currentCompositeTotalCost += childCalculated.cost * childScale;
                    for (const atomicID in childCalculated.optimal) {
                        currentCompositeOptimalPurchase[atomicID] = (currentCompositeOptimalPurchase[atomicID] || 0) + (childCalculated.optimal[atomicID] * childScale);
                    }
                } else {
                    // This implies a topological sort error or missing child. Should not happen if inDegree is correct.
                    console.error(`Error: Child CompositeOrder '${childCompID}' not calculated before parent '${currentCompositeID}'.`);
                }
            }
            ...
            await this.compositeOrders.updateOne(
                ...
                { $set: { totalCost: currentCompositeTotalCost, optimalPurchase: currentCompositeOptimalPurchase } }
            );
      ...
      return {
        error: `Failed to calculate optimal purchase: ${this.getErrorMessage(e)}`,
      };
    }
  }
  ...
  async purchaseOrder(input: PurchaseOrderInput): Promise<PurchaseOrderOutput> {
    try {
      ...
      if (existingCompositeOrder.rootOrder !== compositeOrderID) {
        return {
          error: `CompositeOrder '${compositeOrderID}' is not a root order. Only root orders can be purchased.`,
        };
      }
      ...
      if (existingCompositeOrder.purchased) {
        return {
          error: `CompositeOrder '${compositeOrderID}' has already been purchased.`,
        };
      }
      ...
      if (existingCompositeOrder.totalCost === 0 && Object.keys(existingCompositeOrder.optimalPurchase).length > 0) {
          return { error: `CompositeOrder '${compositeOrderID}' cannot be purchased; some required SelectOrders may lack valid AtomicOrder options or lead to zero cost purchases with positive quantities.` };
      }
      ...
      while (queue.length > 0) {
        ...
        if (currentComposite) {
          for (const childID of Object.keys(currentComposite.childCompositeOrders)) {
  ...
  async _getOrderByAssociateID(
    ...
    try {
      ...
      return {
        error: `Failed to retrieve order by associateID: ${this.getErrorMessage(e)}`,
      };
    }
  }
  ...
  async _getOptimalPurchase(
    ...
    try {
      ...
      return {
        error: `Failed to retrieve optimal purchase: ${this.getErrorMessage(e)}`,
      };
    }
  }
  ...
  async _getOrderCost(input: GetOrderCostInput): Promise<GetOrderCostOutput> {
    try {
      ...
      return { error: `Failed to retrieve order cost: ${this.getErrorMessage(e)}` };
    }
  }
}
```
