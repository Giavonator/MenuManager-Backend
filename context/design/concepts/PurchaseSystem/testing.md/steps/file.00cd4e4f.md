...
content_id: 00cd4e4f1bc03f414fd052916f1960fca989734b53185b9e1ded48b00375b306
---
...
Deno.test("PurchaseSystemConcept - Operating Principle Verification", async (t) => {
  ...
  await t.step("6. Verify Optimal Purchase and Cost", async () => {
    ...
    console.log(
      `    ✅ Check ${++checkIndex}: Total cost is correct: $${totalCost}`,
    );
  });
  ...
  await t.step(
    ...
    async () => {
      ...
      console.log(
        `    ✅ Check ${++checkIndex}: Total cost remains $${costAfterDelete} after child modification.`,
      );
    },
  );
...
Deno.test("PurchaseSystemConcept - Composite Hierarchy and Cycle Prevention", async (t) => {
  ...
  await t.step(
    "1. Create Meal Plan (Root Composite) and Two Recipes (Child Composites)",
    async () => {
      const stepMessage =
        "1. Create Meal Plan (Root Composite) and Two Recipes (Child Composites)";
      printStepHeader(stepMessage);
  ...
  await t.step(
    "3. Attempt to add Meal Plan as a child of Lunch (should form a cycle)",
    async () => {
      const stepMessage =
        "3. Attempt to add Meal Plan as a child of Lunch (should form a cycle)";
      printStepHeader(stepMessage);
