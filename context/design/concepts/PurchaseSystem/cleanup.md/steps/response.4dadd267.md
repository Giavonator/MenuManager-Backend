---
timestamp: 'Thu Oct 23 2025 14:54:18 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251023_145418.76caed70.md]]'
content_id: 4dadd267e734a72073a5d0930989dff7b161d70ed657c79c5c630e8de8fe2fd2
---
...
The concept section gives the *name* of the concept, and a list of *type* *parameters*. These type parameters are for the types of objects that are created externally to the concept, must be treated completely polymorphically by the concept (that is, the concept can't assume that they have any properties at all and can only be compared to determine if two instances of the type are the same identifier/reference and thus represent the same object).
...
**concept** PurchaseSystem \[PurchaseOptionID, AssociatedEntityID]
...
**principle** When a new recipe is created (in CookBook), a `CompositeOrder` is created representing the recipe, containing `SelectOrder`s for each ingredient, potentially scaled. Each `SelectOrder` may have multiple `AtomicOrder` options (from `StoreCatalog`) describing different package sizes/prices. When this recipe `CompositeOrder` is added to a menu `CompositeOrder` (in MenuCollection), a sync ensures the menu's `CompositeOrder` `addSubOrder` of the recipe's `CompositeOrder` with a scale factor. Similarly, when a menu `CompositeOrder` is added to a cart `CompositeOrder` (in ShoppingCart). An administrator then calls `calculateOptimalPurchase` on a top-level `CompositeOrder` (e.g., the cart) to determine the cheapest `AtomicOrder` for each required `SelectOrder`, propagating costs up the hierarchy. Finally, the administrator can call `purchaseOrder` on the top-level `CompositeOrder` once shopping is complete, marking all nested `SelectOrder`s as purchased.

**state**\
  a set of AtomicOrder with\
    a purchaseOptionID PurchaseOptionID // ID of its respective PurchaseOption from StoreCatalog\
    a quantity Float // Ex. 3.0\
    a units String // Ex. "lbs", "oz", "count"\
    a price Float // Ex. 5.99 (cost for this specific quantity)\
    a parentSelectOrder SelectOrder // ID of its single parent SelectOrder\
\
  a set of SelectOrder with\
    a desiredQuantity Float // Ex. 5.0\
    a desiredUnits String // Ex. "lbs"\
    an availableAtomicOrders Set of AtomicOrder // IDs of AtomicOrders that are options for this SelectOrder\
    an optimalAtomicOrder AtomicOrder // ID of the chosen AtomicOrder for optimal purchase, null if not yet calculated\
    a purchased Bool // Flag indicating if the selected optimal AtomicOrder for this SelectOrder has been finalized\
    a parentCompositeOrders Set of CompositeOrder // IDs of CompositeOrders that aggregate this one\
\
  a set of CompositeOrder with\
    an associatedEntityID AssociatedEntityID // Opaque ID (e.g., RecipeID, MenuID, CartID) indicating what this order directly represents\
    a subOrders Map of (SelectOrder | CompositeOrder) to Float // Map from Order ID to its scale factor\
    a parentCompositeOrders Set of CompositeOrder // IDs of CompositeOrders that aggregate this one\
    a calculatedTotalCost Float // Total calculated cost after optimal purchase determination\
    an allSubOrdersResolvedAndPurchased Bool // Flag indicating if all nested SelectOrders are purchased

**actions**\
  createAtomicOrder (purchaseOptionID: PurchaseOptionID, quantity: Float, units: String, price: Float, parentSelectOrder: SelectOrder): (atomicOrder: AtomicOrder)\
    **requires** `purchaseOptionID` exists (in StoreCatalog). `parentSelectOrder` exists.\
    **effects** Creates a new `AtomicOrder` with the given properties and associates it with `parentSelectOrder`. Adds the new `AtomicOrder` to `parentSelectOrder.availableAtomicOrders`. Returns the new `AtomicOrder` ID.

  createSelectOrder (desiredQuantity: Float, desiredUnits: String): (selectOrder: SelectOrder)\
    **requires** `desiredQuantity` > 0.\
    **effects** Creates a new `SelectOrder` with `desiredQuantity`, `desiredUnits`. Initializes `purchased` to `false`, `optimalAtomicOrder` to null. Returns the new `SelectOrder` ID.

  createCompositeOrder (associatedEntityID: AssociatedEntityID): (compositeOrder: CompositeOrder)\
    **requires** `associatedEntityID` is a valid opaque ID (e.g., a RecipeID).\
    **effects** Creates a new `CompositeOrder` with `associatedEntityID`. Initializes `subOrders` to empty, `calculatedTotalCost` to 0.0, and `allSubOrdersResolvedAndPurchased` to `false`. Returns the new `CompositeOrder` ID.
...
    **effects** Adds `atomicOrder` to `selectOrder.availableAtomicOrders`.

  removeAvailableAtomicOrderFromSelectOrder (selectOrder: SelectOrder, atomicOrder: AtomicOrder)\
    **requires** `selectOrder` exists. `atomicOrder` exists and is in `selectOrder.availableAtomicOrders`. If `atomicOrder` is `selectOrder.optimalAtomicOrder`, then `selectOrder.optimalAtomicOrder` is null.\
    **effects** Removes `atomicOrder` from `selectOrder.availableAtomicOrders`. If `atomicOrder` was `selectOrder.optimalAtomicOrder`, it is cleared. Triggers recalculation of optimal purchase and costs in `selectOrder`'s parent `CompositeOrder`s.

  addSubOrder (parentCompositeOrder: CompositeOrder, subOrder: (SelectOrder | CompositeOrder), scaleFactor: Float)\
    **requires** `parentCompositeOrder` exists. `subOrder` exists. `scaleFactor` > 0. `subOrder` is not already a sub-order of `parentCompositeOrder`.\
    **effects** Adds `subOrder` to `parentCompositeOrder.subOrders` with the given `scaleFactor`. Adds `parentCompositeOrder` to `subOrder.parentCompositeOrders`. Triggers recalculation of `calculatedTotalCost` and `allSubOrdersResolvedAndPurchased` up the `parentCompositeOrder` hierarchy.

  removeSubOrder (parentCompositeOrder: CompositeOrder, subOrder: (SelectOrder | CompositeOrder))\
    **requires** `parentCompositeOrder` exists. `subOrder` exists and is a sub-order of `parentCompositeOrder`.\
    **effects** Removes `subOrder` from `parentCompositeOrder.subOrders`. Removes `parentCompositeOrder` from `subOrder.parentCompositeOrders`. Triggers recalculation of `calculatedTotalCost` and `allSubOrdersResolvedAndPurchased` up the `parentCompositeOrder` hierarchy.

  updateSubOrderScaleFactor (parentCompositeOrder: CompositeOrder, subOrder: (SelectOrder | CompositeOrder), newScaleFactor: Float)\
    **requires** `parentCompositeOrder` exists. `subOrder` exists and is a sub-order of `parentCompositeOrder`. `newScaleFactor` > 0.\
    **effects** Updates the `scaleFactor` for `subOrder` in `parentCompositeOrder.subOrders`. Triggers recalculation of `calculatedTotalCost` and `allSubOrdersResolvedAndPurchased` up the `parentCompositeOrder` hierarchy.

  calculateOptimalPurchase (compositeOrder: CompositeOrder)\
    **requires** `compositeOrder` exists. All nested `SelectOrder`s have `availableAtomicOrders`.\
    **effects** Recursively traverses the `subOrders` tree. For each encountered `SelectOrder`, calculates its effective desired quantity and units based on accumulated scale factors. Determines the `AtomicOrder` from its `availableAtomicOrders` that fulfills the effective desired quantity for the least cost (e.g., best unit price among viable options), and sets this as `selectOrder.optimalAtomicOrder`. Updates `calculatedTotalCost` for `compositeOrder` and all its `parentCompositeOrders`. Sets `allSubOrdersResolvedAndPurchased` to `false` for `compositeOrder` and all parent `CompositeOrder`s (since optimal selection might change prior purchase state).

  purchaseOrder (compositeOrder: CompositeOrder)\
    **requires** `compositeOrder` exists. `compositeOrder.parentCompositeOrders` is empty (it is a head node). All `SelectOrder`s nested under `compositeOrder` have an `optimalAtomicOrder` selected.\
    **effects** Recursively traverses the `subOrders` tree. For each `SelectOrder`, sets `selectOrder.purchased` to `true`. Sets `compositeOrder.allSubOrdersResolvedAndPurchased` to `true`.

  deleteOrder (order: (AtomicOrder | SelectOrder | CompositeOrder))\
    **requires** `order` exists.\
    **effects** Deletes the specified `order`.
    *   If `order` is an `AtomicOrder`, it is removed from its `parentSelectOrder.availableAtomicOrders`. If it was the `optimalAtomicOrder` for its parent, that selection is cleared.
    *   If `order` is a `SelectOrder`, it is removed from all its `parentCompositeOrders.subOrders`, and all associated `AtomicOrder`s (whose `parentSelectOrder` is this `SelectOrder`) are deleted.
    *   If `order` is a `CompositeOrder`, it is removed from all its `parentCompositeOrders.subOrders`, and its references to its own `subOrders` are removed (i.e., entries in `subOrders` map are deleted).
    Triggers recalculation in any affected parent CompositeOrders for total cost and purchase status.

**queries**\
  \_getAtomicOrderDetails (atomicOrder: AtomicOrder): (purchaseOptionID: PurchaseOptionID, quantity: Float, units: String, price: Float, parentSelectOrder: SelectOrder)\
    **requires** `atomicOrder` exists.\
    **effects** Returns the details of the `atomicOrder`.

  \_getSelectOrderDetails (selectOrder: SelectOrder): (desiredQuantity: Float, desiredUnits: String, availableAtomicOrders: Set of AtomicOrder, optimalAtomicOrder: AtomicOrder, purchased: Bool, parentCompositeOrders: Set of CompositeOrder)\
    **requires** `selectOrder` exists.\
    **effects** Returns the details of the `selectOrder`.

  \_getCompositeOrderDetails (compositeOrder: CompositeOrder): (associatedEntityID: AssociatedEntityID, subOrders: Map of (SelectOrder | CompositeOrder) to Float, calculatedTotalCost: Float, allSubOrdersResolvedAndPurchased: Bool, parentCompositeOrders: Set of CompositeOrder)\
    **requires** `compositeOrder` exists.\
    **effects** Returns the main details of the `compositeOrder`.

  \_getHeadCompositeOrders (): (compositeOrders: Set of CompositeOrder)\
    **requires** true.\
    **effects** Returns the set of all `CompositeOrder`s that have no `parentCompositeOrders`.

  \_getUnpurchasedHeadCompositeOrders (): (compositeOrders: Set of CompositeOrder)\
    **requires** true.\
    **effects** Returns the set of all `CompositeOrder`s that have no `parentCompositeOrders` and where `allSubOrdersResolvedAndPurchased` is `false`.
