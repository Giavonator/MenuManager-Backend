...
    **effects** Creates a new `AtomicOrder` entity. Its `orderID` is generated, and its `parentSelectOrder` is initially null. Returns the new `AtomicOrder` entity.

  createSelectOrder (desiredQuantity: Float, desiredUnits: String): (selectOrder: SelectOrder)\
    **requires** `desiredQuantity` > 0.\
    **effects** Creates a new `SelectOrder` entity with `desiredQuantity`, `desiredUnits`. Its `orderID` is generated. Initializes `purchased` to `false`. Returns the new `SelectOrder` entity.

  createCompositeOrder (): (compositeOrder: CompositeOrder)\
    **requires** true.\
    **effects** Creates a new `CompositeOrder` entity. Its `orderID` is generated. Initializes `childOrders` to empty, `totalCost` to 0.0, and `purchased` to `false`. Returns the new `CompositeOrder` entity.
