---
timestamp: 'Thu Oct 23 2025 07:17:44 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251023_071744.0497167d.md]]'
content_id: 1bbad6cd81524c7c4c308978a05cfeac8b259f41ecc6356aa323adee120fef81
---
...
**actions**
  createRecipe (name: String): (recipe: Recipe)
    ...
    **effects** Creates a new `Recipe` with the given `name`, empty instructions, default `servingQuantity` \
      (e.g., 1), empty `dishType`, no owner, no associated order, and no ingredients. Returns the new `Recipe` ID.
  ...
  duplicateRecipe (originalRecipe: Recipe, user: User, newName: String): (newRecipe: Recipe)
    ...
    **effects** Creates a new `Recipe` that is a copy of `originalRecipe` (name, instructions, \
      ...
      The new recipe will have no associated order initially. Sets `newRecipe.owner` to `user` and \
      `newRecipe.name` to `newName`. Returns the `newRecipe` ID.
