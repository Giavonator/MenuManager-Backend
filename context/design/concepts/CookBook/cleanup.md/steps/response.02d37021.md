---
timestamp: 'Thu Oct 23 2025 07:13:51 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251023_071351.c06fd0b2.md]]'
content_id: 02d370211ba90fee2e6836c73892a9d1f513df753dc11218a054dd9421eb54b4
---
...
**state**
  a set of Recipe with
    ...
    a dishType String
    an owner User
    a purchaseOrder Order // Added: Each Recipe has an associated purchase Order
...
**actions**
  createRecipe (name: String): (recipe: Recipe)
    ...
    **effects** Creates a new `Recipe` with the given `name`, empty instructions, default `servingQuantity`
      (e.g., 1), empty `dishType`, no owner, no purchase order, and no ingredients. Returns the new `Recipe` ID.
  ...
  duplicateRecipe (originalRecipe: Recipe, user: User, newName: String): (newRecipe: Recipe)
    ...
    **effects** Creates a new `Recipe` that is a copy of `originalRecipe` (name, instructions,
      ...
      Sets `newRecipe.owner` to `user` and `newRecipe.name` to `newName`. The new recipe has no purchase order. Returns the `newRecipe` ID.
  ...
  addOrderToRecipe (recipe: Recipe, order: Order) // Added: Action to assign an Order to a Recipe
    **requires** `recipe` exists. `order` exists.
    **effects** Sets `recipe.purchaseOrder` to `order`.

**queries**
  _getRecipeDetails (recipe: Recipe): (name: String, instructions: String, servingQuantity: Int, dishType: String, owner: User, purchaseOrder: Order)
    **requires** `recipe` exists.
    **effects** Returns all primary attributes of the recipe, including its associated purchase order.
