---
timestamp: 'Thu Oct 23 2025 07:12:54 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251023_071254.82f36432.md]]'
content_id: 5057b0009fc5a188d8f3674436d9cf170fbf070ef25a5b110514f4c764a8ffae
---
...
      (e.g., 1), empty `dishType`, no owner, no order, and no ingredients. Returns the new `Recipe` ID.
...
    **effects** Sets `recipe.order` to `order`. If an `order` was already associated, it is replaced.
...
      servingQuantity, dishType, owner, order, and all ingredients including their names, \
      quantities, and units). Sets `newRecipe.owner` to `user` and `newRecipe.name` to `newName`. \
      Returns the `newRecipe` ID.
...
    **effects** Returns all primary attributes of the recipe, including its associated order.
