...
content_id: d46461ca05c248741bb7f1f70cd795e8ef59ad415437ac0b7d1cef14c0cab42d
---
...
export default class CookBookConcept {
   ...
   */
  async createRecipe({ name, user }: CreateRecipeInput): Promise<CreateRecipeOutput> {
    if (!name || name.trim() === "") {
    ...
    try {
      ...
      return { error: `Failed to create recipe: ${e.message}` };
    }
  }
  ...
  async updateRecipe(input: UpdateRecipeInput): Promise<UpdateRecipeOutput> {
    ...
    if ("instructions" in input) {
      ...
      if (typeof input.servingQuantity !== "number" || input.servingQuantity <= 0) {
    ...
    try {
      ...
      return { error: `Failed to update recipe: ${e.message}` };
    }
  }
  ...
  async duplicateRecipe({
    ...
    try {
      ...
      const existingRecipe = await this.recipes.findOne({ name: newName.trim(), owner: user });
      if (existingRecipe) {
        return {
          error: `Recipe with name '${newName}' already exists for user '${user}'.`,
        };
      }
      ...
      console.error(`Error duplicating recipe: ${e.message}`);
      return { error: `Failed to duplicate recipe: ${e.message}` };
    }
  }
  ...
  async addRecipeIngredient({
    ...
    try {
      ...
      const ingredientExists = existingRecipe.ingredients.some((ing) => ing.name === name.trim());
      if (ingredientExists) {
        return { error: `Ingredient '${name}' already exists in recipe '${recipe}'.` };
      }
      ...
      console.error(`Error adding ingredient to recipe: ${e.message}`);
      return { error: `Failed to add ingredient to recipe: ${e.message}` };
    }
  }
  ...
  async updateRecipeIngredient({
    ...
    try {
      ...
      if (result.matchedCount === 0) {
        return { error: `Recipe '${recipe}' or ingredient '${name}' not found.` };
      }
      return {};
    } catch (e) {
      console.error(`Error updating ingredient in recipe: ${e.message}`);
      return { error: `Failed to update ingredient in recipe: ${e.message}` };
    }
  }
  ...
  async removeRecipeIngredient({
    ...
    try {
      ...
      console.error(`Error removing ingredient from recipe: ${e.message}`);
      return { error: `Failed to remove ingredient from recipe: ${e.message}` };
    }
  }
   ...
   */
  async _getRecipeDetails({ recipe }: GetRecipeDetailsInput): Promise<GetRecipeDetailsOutput> {
    if (!recipe || recipe.trim() === "") {
      return { error: "Recipe ID cannot be empty." };
    }
    ...
    try {
      ...
      console.error(`Error getting recipe details: ${e.message}`);
      return { error: `Failed to retrieve recipe details: ${e.message}` };
    }
  }
  ...
  async _getRecipeIngredients({
    ...
    try {
      ...
      console.error(`Error getting recipe ingredients: ${e.message}`);
      return { error: `Failed to retrieve recipe ingredients: ${e.message}` };
    }
  }
  ...
  async _getRecipesOwnedByUser({
    ...
    try {
      ...
      console.error(`Error getting recipes owned by user: ${e.message}`);
      return { error: `Failed to retrieve recipes for user: ${e.message}` };
    }
  }
}
```
