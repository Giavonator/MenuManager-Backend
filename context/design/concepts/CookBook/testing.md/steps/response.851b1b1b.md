---
timestamp: 'Fri Oct 24 2025 06:50:36 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251024_065036.4af1d303.md]]'
content_id: 851b1b1bd729c7652dcebc6d501f1c3586ee3cd8125aed68e576b118056b0ffe
---
...
Deno.test("CookBookConcept - Operational Principle Verification", async (t) => {
  ...
  await t.step("2. Alice adds ingredients to 'Spicy Pasta'", async () => {
    ...
    assertAndLog(
      ingredients[0].ingredients.some((i) => i.name === "Pasta"),
      true,
  ...
  await t.step(
    ...
    async () => {
      ...
      const bobRecipes = await cookBook._getRecipesOwnedByUser({ user: chefBob });
      assertAndLog(
        "error" in bobRecipes,
        false,
      ...
      assertAndLog(
        mildPastaDetails[0].instructions,
        "Boil pasta. Add sauce. Serve hot.",
  ...
  await t.step(
    ...
    async () => {
      ...
      assertAndLog(
        mildPastaIngredients[0].ingredients.length,
      ...
      assertAndLog(
        mildPastaIngredients[0].ingredients.some((i) => i.name === "Chili Flakes"),
        false,
      ...
      assertAndLog(
        mildPastaIngredients[0].ingredients.some((i) => i.name === "Cream"),
        true,
      ...
      assertAndLog(
        mildPastaIngredients[0].ingredients.find((i) => i.name === "Pasta")
          ?.quantity,
        300,
  ...
  await t.step(
    ...
    async () => {
      const stepMessage = "6. Verify Alice's original 'Spicy Pasta' is unchanged";
      printStepHeader(stepMessage);
      let checkIndex = 0;

      const aliceRecipes = await cookBook._getRecipesOwnedByUser({ user: chefAlice });
      assertAndLog(
        aliceRecipes.length,
        1,
      ...
      assertAndLog(
        spicyPastaIngredients[0].ingredients.length,
      ...
      assertAndLog(
        spicyPastaIngredients[0].ingredients.some((i) => i.name === "Chili Flakes"),
        true,
      ...
      assertAndLog(
        spicyPastaIngredients[0].ingredients.some((i) => i.name === "Cream"),
        false,
      ...
      assertAndLog(
        spicyPastaIngredients[0].ingredients.find((i) => i.name === "Pasta")
          ?.quantity,
        500,
...
Deno.test("CookBookConcept - Action Specific Validations and Edge Cases", async (t) => {
  ...
  await t.step("createRecipe - Valid Creation", async () => {
    ...
    assertExistsAndLog(recipeId, "Recipe ID should be returned", stepMessage, ++checkIndex);

    const details = await cookBook._getRecipeDetails({ recipe: recipeId });
    assertAndLog(details[0].name, "Chocolate Cake", "Name should be correct", stepMessage, ++checkIndex);
    assertAndLog(details[0].owner, testUser, "Owner should be correct", stepMessage, ++checkIndex);
    assertAndLog(details[0].servingQuantity, 1, "Default serving quantity should be 1", stepMessage, ++checkIndex);
  });
  ...
  await t.step("createRecipe - Invalid Inputs (requires)", async () => {
    ...
    const whitespaceNameResult = await cookBook.createRecipe({ name: "   ", user: testUser });
    assertAndLog(
    ...
    const emptyUserResult = await cookBook.createRecipe({ name: "Another Recipe", user: "" as ID });
    assertAndLog(
  ...
  await t.step("updateRecipe - Valid Updates (effects)", async () => {
    ...
    const updateNameResult = await cookBook.updateRecipe({ recipe: recipeId, name: "Delicious Cake" });
    assertAndLog("error" in updateNameResult, false, "Should successfully update name", stepMessage, ++checkIndex);
    let details = await cookBook._getRecipeDetails({ recipe: recipeId });
    assertAndLog(details[0].name, "Delicious Cake", "Name should be updated", stepMessage, ++checkIndex);

    const updateInstructionsResult = await cookBook.updateRecipe({ recipe: recipeId, instructions: "Mix, bake, enjoy." });
    assertAndLog("error" in updateInstructionsResult, false, "Should successfully update instructions", stepMessage, ++checkIndex);
    details = await cookBook._getRecipeDetails({ recipe: recipeId });
    assertAndLog(details[0].instructions, "Mix, bake, enjoy.", "Instructions should be updated", stepMessage, ++checkIndex);

    const updateServingResult = await cookBook.updateRecipe({ recipe: recipeId, servingQuantity: 8 });
    assertAndLog("error" in updateServingResult, false, "Should successfully update serving quantity", stepMessage, ++checkIndex);
    details = await cookBook._getRecipeDetails({ recipe: recipeId });
    assertAndLog(details[0].servingQuantity, 8, "Serving quantity should be updated", stepMessage, ++checkIndex);

    const updateDishTypeResult = await cookBook.updateRecipe({ recipe: recipeId, dishType: "Dessert" });
    assertAndLog("error" in updateDishTypeResult, false, "Should successfully update dish type", stepMessage, ++checkIndex);
    details = await cookBook._getRecipeDetails({ recipe: recipeId });
    assertAndLog(details[0].dishType, "Dessert", "Dish type should be updated", stepMessage, ++checkIndex);
  });
  ...
  await t.step("updateRecipe - Invalid Updates (requires)", async () => {
    ...
    const invalidRecipeIdResult = await cookBook.updateRecipe({ recipe: "nonExistent" as ID, name: "Bad Recipe" });
    assertAndLog(
    ...
    const invalidServingQuantityResult = await cookBook.updateRecipe({ recipe: recipeId, servingQuantity: 0 });
    assertAndLog(
    ...
    const emptyNameUpdateResult = await cookBook.updateRecipe({ recipe: recipeId, name: " " });
    assertAndLog(
  ...
  await t.step("addRecipeIngredient - Invalid Additions (requires)", async () => {
  ...
  await t.step("updateRecipeIngredient - Valid Update (effects)", async () => {
    ...
    await cookBook.addRecipeIngredient({ recipe: recipeId, name: "Eggs", quantity: 3, units: "large" });
    ...
    const ingredients = await cookBook._getRecipeIngredients({ recipe: recipeId });
    const flour = ingredients[0].ingredients.find((i) => i.name === "Flour");
    assertExistsAndLog(flour, "Flour ingredient should exist", stepMessage, ++checkIndex);
    assertAndLog(fl
      ?.quantity,
      300,
  ...
  await t.step("updateRecipeIngredient - Invalid Updates (requires)", async () => {
  ...
  await t.step("removeRecipeIngredient - Valid Removal (effects)", async () => {
    ...
    await cookBook.addRecipeIngredient({ recipe: recipeId, name: "Sugar", quantity: 150, units: "g" });
    let ingredients = await cookBook._getRecipeIngredients({ recipe: recipeId });
    assertAndLog(ingredients[0].ingredients.length, 3, "Recipe should have 3 ingredients before removal", stepMessage, ++checkIndex);
    ...
    assertAndLog(ingredients[0].ingredients.length, 2, "Recipe should have 2 ingredients after removal", stepMessage, ++checkIndex);
    assertAndLog(
      ingredients[0].ingredients.some((i) => i.name === "Flour"),
      false,
  ...
  await t.step("removeRecipeIngredient - Invalid Removals (requires)", async () => {
    const stepMessage = "removeRecipeIngredient - Invalid Removals (requires)";
    printStepHeader(stepMessage);
    let checkIndex = 0;
  ...
  await t.step("Queries - Error Cases", async () => {
    ...
    const detailsResult = await cookBook._getRecipeDetails({ recipe: "badID" as ID });
    assertAndLog("error" in detailsResult, true, "Details query for bad ID should fail", stepMessage, ++checkIndex);
    assertAndLog(
    ...
    const ingredientsResult = await cookBook._getRecipeIngredients({ recipe: "badID" as ID });
    assertAndLog("error" in ingredientsResult, true, "Ingredients query for bad ID should fail", stepMessage, ++checkIndex);
    assertAndLog(
    ...
    const recipesByUserResult = await cookBook._getRecipesOwnedByUser({ user: "" as ID });
    assertAndLog("error" in recipesByUserResult, true, "Recipes by empty user ID should fail", stepMessage, ++checkIndex);
    assertAndLog(
    ...
    const nonExistentUserRecipes = await cookBook._getRecipesOwnedByUser({ user: "nonExistentUser" as ID });
    assertAndLog("error" in nonExistentUserRecipes, false, "Query for non-existent user should return no error but empty array", stepMessage, ++checkIndex);
    assertAndLog(
